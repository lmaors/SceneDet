...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_workers', type=int, default=4, help='number of processors.')
    parser.add_argument('--source_path', type=str, 
                        default=osp.join("videos"),
                        help="path to the videos to be processed, please use absolute path")
    parser.add_argument('--list_file', type=str, 
                        default=osp.join("data/info", "video_list.txt"),
    ...
    parser.add_argument('--save_keyf_txt',   action="store_true", default=True)
    parser.add_argument('--split_video',     action="store_true", default=True)
    ...
    videos = [ i  for i in os.listdir(args.source_path) if os.path.splitext(i)[1] == '.mp4' ]
    with open('data/info/video_list.txt','w') as fw:
        for n,video in enumerate(videos):
            fw.write('\n'+video) if n != 0 else  fw.write(video) 
    print('\033[1;36m Video list has updated.\033[0m')
