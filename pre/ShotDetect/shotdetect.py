...
def shots_detect(args, data_root,video):


    # video_path = osp.abspath(args.video_path)
    video_path = osp.join(data_root,'videos',video)
...
def main(args):
    assert osp.isfile(args.list_file)
    ...
    for num,video in enumerate(video_list):
        print('It is processing the video {}'.format(num+1))

        shots_detect(args, args.save_data_root_path,video.strip())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--list_file', type=str,
                        default=osp.join(myexp_path, 'meta', 'videos_list.txt'),
