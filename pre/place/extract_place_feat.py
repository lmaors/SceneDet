...
def main(args):
    ...
    video_list = [i.split(".")[0] for i in video_list] 
...
if __name__ == '__main__':
    ...
    data_root = osp.dirname(osp.dirname(osp.dirname(__file__)))
    myexp_path = osp.join(data_root, my_experiment_dir)
    parser = argparse.ArgumentParser("Place feature using ResNet50 with ImageNet pretrain")
    parser.add_argument('--save-one-frame-feat', action="store_true",default=True)
    parser.add_argument('-b', '--batch-size', type=int, default=16) # 128
    parser.add_argument('-j', '--workers', type=int, default=32)
    parser.add_argument('--list_file', type=str,default=osp.join(myexp_path,'info','video_list.txt'),
                        help='The list of videos to be processed,\
                        in the form of xxxx0.mp4\nxxxx1.mp4\nxxxx2.mp4\n \
                                     or xxxx0\nxxxx1\nxxxx2\n' )
    parser.add_argument('--source_img_path', type=str,default=osp.join(myexp_path,'shot_keyf'))
    parser.add_argument('--save_path',type=str,default=osp.join(myexp_path,'place_feat_raw'))
    parser.add_argument('--save_feat_path',type=str,default=osp.join(myexp_path,'place_feat'))
