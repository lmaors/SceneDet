...
from  utils.utils import read_txt_list,mkdir_ifmiss,write_json
import cv2
...
def get_mAP(loader, gts_raw, preds_raw):
    ...
    lines = []
    for i in range(n):
        one_idx = loader.dataset.list_ids[i]
...
def get_mAP_seq(loader, gts_raw, preds_raw):
    ...
    seq_len = len(loader.dataset.list_ids[0])
...
def save_pred_seq(cfg,loader,gts,preds):
    ...
    for threshold in np.arange(0,1.01,0.05):
        ...
        n = min(len(loader.dataset.list_ids), len(gts)//cfg.seq_len, len(preds)//cfg.seq_len)
